...
export const newMessageToast = (message: MessageDTO) => {
  toast(
    <NotificationToast 
      ...
      href={`/dogs/${message.senderId}/chat`}
      title={`${message.senderName} has send you a new message`}
    />
  )
}
...
export const newLikeToast = (name: string, image: string | null, userId: string) => {
  toast(
    <NotificationToast 
      image={image}
      href={`/dogs/${userId}`}
      title={`Woof! You have been liked by ${name}`}
      subtitle='Click here to sniff around their profile'
    />
  )
}
